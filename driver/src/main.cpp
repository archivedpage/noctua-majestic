...
namespace driver
{
    ...
    NTSTATUS deviceControl(const PDEVICE_OBJECT device_object, const PIRP irp)
    {
        ...
        case control_codes::read:
            ...
            {
                // Copy from target process into the requester user buffer safely
                ...
                {
                    dbgPrint("[-] control_codes::read -> IoGetRequestorProcess failed\n");
                }
            }
            else
            {
        ...
        case control_codes::write:
            if (targetProcess != nullptr)
            {
                // Copy from requester user buffer into target process safely
                PEPROCESS requester = IoGetRequestorProcess(irp);
                if (requester)
                {
                    status = MmCopyVirtualMemory(
                ...
                {
                    dbgPrint("[-] control_codes::write -> IoGetRequestorProcess failed\n");
                }
            }
            else
            {
            ...
            break;
        case control_codes::get_image_base:
            if (targetProcess != nullptr)
            {
                request->target_address = PsGetProcessSectionBaseAddress(targetProcess);
                status = STATUS_SUCCESS;
                request->return_size = sizeof(request->target_address);
            }
            else
            {
                dbgPrint("[-] control_codes::get_image_base -> targetProcess is null\n");
            }
            break;
        default:
            break;
