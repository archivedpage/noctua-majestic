...
namespace driver
{
    ...
    namespace control_codes
    {
        ...
        constexpr ULONG get_image_base{CTL_CODE(FILE_DEVICE_UNKNOWN, 0xA4, METHOD_BUFFERED, FILE_SPECIAL_ACCESS)};
    }
    ...
    public:
        ...
        [[nodiscard]] bool get_image_base(std::uintptr_t& out_base) const;
        ...
        bool read_buf(const std::uintptr_t& address, void* buffer, size_t size) const
        {
            Request driver_request{};
            driver_request.target_address = reinterpret_cast<PVOID>(address);
            driver_request.buffer = buffer;
            driver_request.size = size;
            return DeviceIoControl(handle_, control_codes::read, &driver_request, sizeof(driver_request), &driver_request, sizeof(driver_request), nullptr, nullptr) != FALSE;
        }

        bool write_buf(const std::uintptr_t& address, const void* buffer, size_t size) const
        {
            Request driver_request{};
            driver_request.target_address = reinterpret_cast<PVOID>(address);
            driver_request.buffer = const_cast<void*>(buffer);
            driver_request.size = size;
            return DeviceIoControl(handle_, control_codes::write, &driver_request, sizeof(driver_request), &driver_request, sizeof(driver_request), nullptr, nullptr) != FALSE;
        }
