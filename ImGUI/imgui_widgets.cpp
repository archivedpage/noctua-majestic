...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        ...
        window->scroll_y = *p_scroll_v = (ImS64)(scroll_v_norm * scroll_max);
...
void ImGui::Combo( const char* label, int* v, std::vector< const char* > items ) {
	...
	auto id = window->GetID( label );
	auto& style = GetStyle( );
    int search = GImGui->CurrentItemFlags & ImGuiItemFlags_Search;
    ...
    struct s {
        ...
        ImColor col = 0;
        float w = 0;
    }; auto& obj = anim_obj( label, 0 + search, s{ } );

    obj.w = ImLerp( obj.w, CalcTextSize( items[*v], 0, 1 ).x, GetIO( ).DeltaTime * 14 );

	ImVec2 pos = window->DC.CursorPos;
	ImVec2 label_size = CalcTextSize( label, 0, 1 );
	ImRect total_bb( pos, pos + ImVec2( CalcItemWidth( ), GImGui->FontSize ) );
	ImRect bb( total_bb.Min + ImVec2{ total_bb.GetWidth( ) - 16 - ( int )obj.w, 0 }, total_bb.Max );
    ...
    ui::rotate_start( );
    RenderArrow( window->DrawList, { bb.Max.x - 11, bb.GetCenter( ).y - 4 - obj.anim }, col_anim( col_anim( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_TextHovered ), obj.hover ), GetColorU32( ImGuiCol_Scheme ), obj.anim ), ImGuiDir_Down, 0.7f );
    ...
    window->DrawList->AddText( bb.Min, obj.col, items[*v], FindRenderedTextEnd( items[*v] ) );
    ...
    if ( obj.anim > 0.05f ) {
        ...
        Begin( label, 0, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize );
        {
            SetWindowSize( { 122, ( GetCurrentWindow( )->ContentSize.y + GImGui->Style.WindowPadding.y * 2 ) * obj.anim } );
            SetWindowPos( { bb.Max.x - 122, bb.GetCenter( ).y - GetWindowHeight( ) / 2 * obj.anim } );
            ...
            BeginGroup( );
            {
                for ( int i = 0; i < items.size( ); ++i ) {
                    if ( Selectable( items[i], i == *v, 0, { GetWindowWidth( ) - 20, GImGui->FontSize } ) ) {
                        *v = i;
                        obj.open = false;
                    }
                }
...
bool ImGui::Checkbox(const char* label, bool* v, int* key, std::vector< float* > col, void ( *settings )( ))
{
    return false;
}
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    struct s {
        ...
        float val_anim = 0;
        ImColor col = 0;
    }; auto& obj = anim_obj( label, 3, s{ } );

    obj.anim = anim( obj.anim, 0.f, 1.f, hovered || IsItemActive( ) );
    obj.active = anim( obj.active, 0.f, 1.f, IsItemActive( ) );
    obj.col = col_anim( col_anim( GetColorU32( ImGuiCol_TextDisabled ), GetColorU32( ImGuiCol_TextHovered ), obj.anim ), GetColorU32( ImGuiCol_Text ), obj.active );
    ...
    window->DrawList->AddShadowRect( bb.Min, { bb.Min.x + obj.val_anim * style.Alpha, bb.Max.y }, GetColorU32( ImGuiCol_Scheme, 0.4f ), 30, { 0, 0 }, 0, 20 );
	window->DrawList->AddRectFilled( bb.Min, { bb.Min.x + obj.val_anim * style.Alpha, bb.Max.y }, GetColorU32( ImGuiCol_Scheme, 1.f - 0.4f * obj.active ), 2 );
    window->DrawList->AddCircleFilled( { bb.Min.x + obj.val_anim * style.Alpha, bb.GetCenter( ).y }, 5.5f + obj.anim - 2.f * obj.active, GetColorU32( ImGuiCol_SliderGrab ) );

	window->DrawList->AddText( total_bb.Min, obj.col, label, FindRenderedTextEnd( label ) );
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data, const char* icon)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, CalcItemWidth() * 185 / 300.f, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + style.FramePadding + ImVec2{ 17, 0 } * bool( icon );
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, GetColorU32( is_displaying_hint ? GetStyleColorVec4( ImGuiCol_TextDisabled ) : obj.col.Value ), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, GetColorU32( is_displaying_hint ? GetStyleColorVec4( ImGuiCol_TextDisabled ) : obj.col.Value ), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
    ...
    if ( icon ) {
        ui::add_text( icons, 11, frame_bb.Min + ImVec2{ style.FramePadding.x, frame_bb.GetHeight( ) / 2 - 11.f / 2 }, obj.col, icon );
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    // Render SV Square
    GetForegroundDrawList()->AddRectFilledMultiColorRounded2(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), col_white, hue_color32, hue_color32, col_white, style.ChildRounding, ImDrawCornerFlags_All);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded2(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, style.ChildRounding, ImDrawCornerFlags_All);
    ...
    GetForegroundDrawList()->AddRectFilledMultiColorRounded2(ImVec2(bar0_pos_x, bar_pos.y + 6), ImVec2(bar0_pos_x + bars_width, bar_pos.y + sv_bar_size - 7), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 30.f, ImDrawCornerFlags_All);
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddRectFilledMultiColorRounded2(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 2.f, ImDrawCornerFlags_All);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoBorder) == 0)
    {
        if (g.Style.FrameBorderSize > 0.0f)
            ...
            GetWindowDrawList()->AddShadowRect(bb_inner.Min, bb_inner.Max + ImVec2(0, 0), GetColorU32(col_rgb), 30, ImVec2(0.f, 0.f), 5.f, 16);
